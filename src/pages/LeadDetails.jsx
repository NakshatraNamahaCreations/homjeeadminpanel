...
import EditEnquiryModal from "./EditEnquiryModal"; // ✅ Using SAME modal now
import { BASE_URL } from "../utils/config";

/* -------------------------------------------------
   Convert LEAD → ENQUIRY modal compatible structure
   (left unchanged as requested)
-------------------------------------------------- */
const convertLeadToEnquiry = (lead) => {
  ...
  return {
    ...
    createdDate: lead.createdAtLocalDate,
    createdTime: lead.createdAtLocalTime,
    filledData: lead.filledData,

    raw: {
      ...lead.raw,
      isRead: lead.raw?.isRead ?? false,
      isDismmised: lead.raw?.isDismmised ?? false,
    },
  };
};
...
const LeadDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchVendors = async () => {
      try {
        ...
        const scRaw =
          (lead?.service?.[0]?.category || lead?.serviceType || "").toLowerCase();
        const isDeepCleaning = scRaw.includes("deep cleaning") || scRaw.includes("deep");
  ...
  const handleNotifyVendor = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setNotificationStatus(`Vendor notified successfully for booking ${lead._id || lead.booking_id || ""}.`);
        setLead((prev) => ({
          ...prev,
          bookingDetails: {
            ...(prev?.bookingDetails || {}),
  ...
  const createdAt = lead.createdAt || lead.createdDate || lead.bookingDetails?.createdAt;
    ...
    >
          ...
          <div className="card-body">
            ...
            >
              <div>
                ...
                <p className="fw-bold mb-1">{lead.customer?.name || lead.name}</p>
                ...
                <p className="text-muted mb-1" style={{ fontSize: "14px" }}>
                  <FaPhone className="me-1" /> {lead.customer?.phone || lead.contact}
                </p>
              </div>

              <div className="text-end">
                <p className="text-black mb-0" style={{ fontSize: "12px" }}>
                  {lead.bookingDetails?.bookingDate || lead.selectedSlot?.slotDate || lead.date}
                </p>
                <p className="fw-bold mb-2" style={{ fontSize: "12px" }}>
                  {lead.bookingDetails?.bookingTime || lead.selectedSlot?.slotTime || lead.time}
                </p>
                ...
                <button
                  ...
                  onClick={() => {
                    const lat = lead?.address?.location?.coordinates?.[1] ?? lead?.filledData?.location?.lat;
                    const lng = lead?.address?.location?.coordinates?.[0] ?? lead?.filledData?.location?.lng;
                    if (lat && lng) {
                      window.open(
              ...
              <div className="d-flex flex-column" style={{ width: "50%" }}>
                ...
                >
                  ...
                  <p style={{ fontSize: "12px" }}>
                    ...
                    <strong>{lead.bookingDetails?.booking_id || "HJC66383"}</strong>{" "}
                    <FaCopy
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const result = await response.json();
                      if (response.ok) {
                        setLead({ ...lead, status: "Admin Cancelled", bookingDetails: { ...(lead.bookingDetails || {}), status: "Admin Cancelled" } });
      ...
      {showEditModal && lead && (
        <EditEnquiryModal
          ...
          enquiry={convertLeadToEnquiry(lead)}
          onUpdated={(updatedBooking) => {
            ...
            setLead((prev) => ({
              ...
              filledData: {
                ...prev.filledData,
                location: {
                  name: updatedBooking.address?.streetArea || "",
                  lat: updatedBooking.address?.location?.coordinates?.[1],
                  lng: updatedBooking.address?.location?.coordinates?.[0],
                },
                timeSlot: updatedBooking.selectedSlot?.slotTime,
              },
            }));
          }}
          title="Edit Lead"
