// PART 1
// EditEnquiryModal.jsx - PART 1 (imports, helper, component start, states)
...
// helpers, deepList fetch, load enquiry useEffect
  ...
  useEffect(() => {
    ...
    try {
      ...
      // IMPORTANT: pick finalTotal from backend if present, otherwise fallback to originalTotalAmount
...
// PART 3
// addService, removeService, onServiceChange with leadMode-aware adjustments

  // -------------------------------------------------------------------
  // ADD NEW SERVICE (Deep Cleaning only)
  // In leadMode: when admin adds a service, we expect the price to be set later
  // If price present at add time, adjust serverFinalTotal immediately.
  // -------------------------------------------------------------------
  const addService = () => {
  ...
  // In leadMode: subtract removed service price from serverFinalTotal
  // -------------------------------------------------------------------
  ...
  // In leadMode: if price changes, apply delta to serverFinalTotal.
  // -------------------------------------------------------------------
...
// PART 4
// services change effect (only non-leadMode recompute) and manual final total apply
  ...
  // For leadMode: DO NOT recompute from services — we update finalTotal on add/remove/price-change manually
  // For non-leadMode: keep previous behaviour.
  // -------------------------------------------------------------------
  useEffect(() => {
  ...
  /* -----------------------------------------------------------
     PART 4 — applyManualFinalTotal (manual override)
     - When leadMode === true: manualValue replaces serverFinalTotal
     - Recalculate AYTP/refund based on paidAmount
     - When leadMode === false: keep previous enquiry behaviour
  ----------------------------------------------------------- */
