...
const DeepCleaningDashboard = () => {
  ...
  const fetchPackages = async () => {
    try {
      ...
      // ✅ IMPORTANT: keep same order as API
      const list = (json.data || []).map((p) => ({
        id: p._id || p.id,
        name: p.name,
        category: p.category,
      ...
      setPackages(list); // ✅ API order preserved
    } catch (err) {
      console.error("GET packages error:", err.message);
    } finally {
      setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ✅ IMPORTANT: re-fetch to keep EXACT server order
      await fetchPackages();
              ...
              <tr key={pkg.id}>
                ...
                <td>
                  <div className="d-flex gap-2 justify-content-center">
                    ...
                    {/* If you enable delete later, order will still stay correct due to re-fetch */}
                    {/* <Button
                      variant="outline-danger"
                      onClick={() => handleDelete(pkg)}
                      style={{ fontSize: "12px" }}
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                <Form.Select
                  ...
                  onChange={(e) => onFormChange("service", e.target.value)}
                  disabled={
                    isEditMode || !form.subcategory || serviceOptions.length === 0
                  }
                >
                  <option value="">
                    {serviceOptions.length
